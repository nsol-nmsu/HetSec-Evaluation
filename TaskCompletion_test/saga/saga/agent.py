...
class Agent:
    ...
    def __init__(self, workdir: str, material: dict, local_agent: LocalAgent = None):
        ...
        prov_der = provider_cert.public_bytes(sc.serialization.Encoding.DER)
        logger.log("CRYPTO", f"Provider cert sha256: {hashlib.sha256(prov_der).hexdigest()}")

        prov_pk = provider_cert.public_key().public_bytes(
            encoding=sc.serialization.Encoding.Raw,
            format=sc.serialization.PublicFormat.Raw
        )
        logger.log("CRYPTO", f"Provider pk sha256: {hashlib.sha256(prov_pk).hexdigest()}")        
        ...
        try:
            ...
            logger.log("CRYPTO", f"Stampe verification passed.")
        ...
        self.monitor = Monitor(time.perf_counter)
        self.llm_monitor = Monitor(time.perf_counter)
    ...
    def initiate_conversation(self, conn, token: str, r_aid: str, init_msg: str) -> bool:
        ...
        while True:
            ...
            self.monitor.start("c:communication_conv")
            self.send(conn, msg)
            ...
            if msg['msg'] == self.task_finished_token:
                ...
                self.monitor.stop("c:communication_conv")
                return True
                
            # Receive response:
            response = self.recv(conn)
            self.monitor.stop("c:communication_conv")
            logger.log("OVERHEAD", f"c:communication_conv: {self.monitor.elapsed('c:communication_conv')}")
            with open("overhead_calander.csv","a",newline="") as f:
                f.write(f"\nc:communication_conv: {self.monitor.elapsed('c:communication_conv')}")
            ...
            if i > MAX_QUERIES:
                ...
                return True
            self.llm_monitor.start("t:llm_turn")
            agent_instance, text = self.local_agent.run(received_message, initiating_agent=True, agent_instance=agent_instance)
            self.llm_monitor.stop("t:llm_turn")
            logger.log("OVERHEAD", f"t:llm_turn: {self.llm_monitor.elapsed('t:llm_turn')}")

            with open("overhead_calander.csv","a",newline="") as f:
                f.write(f"\nt:llm_turn: {self.llm_monitor.elapsed('t:llm_turn')}")            
    ...
    def connect(self, r_aid, message: str):
        ...
        self.monitor.start("c:proto_init")
        ...
        if token is not None:
            ...
            self.monitor.stop("c:proto_init")
            self.monitor.start("c:provider_lookup")
            r_agent_material = self.access(r_aid)
            self.monitor.stop("c:provider_lookup")
            logger.log("OVERHEAD", f"c:provider_lookup: {self.monitor.elapsed('c:provider_lookup')}")
            with open("overhead_calander.csv","a",newline="") as f:
                f.write(f"\n\n\nc:provider_lookup: {self.monitor.elapsed('c:provider_lookup')}")    
        ...
        self.monitor.start("c:TLS_Socket_Creation")
        ...
        try:
            ...
            with socket.create_connection((r_ip, r_port)) as sock:
                ...
                with context.wrap_socket(sock, server_hostname=r_aid) as conn:
                    ...
                    self.monitor.stop("c:TLS_Socket_Creation")
                    logger.log("OVERHEAD", f"c:TLS_Socket_Creation: {self.monitor.elapsed('c:TLS_Socket_Creation')}")
                    with open("overhead_calander.csv","a",newline="") as f:
                        f.write(f"\nc:TLS_Socket_Creation: {self.monitor.elapsed('c:TLS_Socket_Creation')}")    
                    ...
                    # Stop the stopwatch
                    self.monitor.stop("c:proto_init")
                    logger.log("OVERHEAD", f"c:proto_init: {self.monitor.elapsed('c:proto_init')}")
                    with open("overhead_calander.csv","a",newline="") as f:
                        f.write(f"\nc:proto_init: {self.monitor.elapsed('c:proto_init')}")

                    # Restart the stopwatch:
                    self.monitor.start("c:communication_init")
                    # Send JSON request
                    ...
                    self.monitor.stop("c:communication_init")
                    logger.log("OVERHEAD", f"c:communication_init: {self.monitor.elapsed('c:communication_init')}")
                    with open("overhead_calander.csv","a",newline="") as f:
                        f.write(f"\nc:communication_init: {self.monitor.elapsed('c:communication_init')}")

                    if token is None and response_dict:
                        # If no valid token was found, the expected response is a token.
                        
                        self.monitor.start("c:token_init")
                        ...
                        self.monitor.stop("c:token_init")
                        logger.log("OVERHEAD", f"c:token_init: {self.monitor.elapsed('c:token_init')}")
                        with open("overhead_calander.csv","a",newline="") as f:
                            f.write(f"\nc:token_init: {self.monitor.elapsed('c:token_init')}")
            ...
            if received_msg:
                    ...
                    try:
                        ...
                        if i_token is None:
                            ...
                            with open("overhead_calander.csv","a",newline="") as f:
                                f.write(f"\nagent:token_recv: {self.monitor.elapsed('agent:token_recv')}")
                            ...
                            with open("overhead_calander.csv","a",newline="") as f:
                                f.write(f"\nagent:communication_proto_recv: {self.monitor.elapsed('agent:communication_proto_recv')}")
                            ...
                            with open("overhead_calander.csv","a",newline="") as f:
                                f.write(f"\nagent:communication_conv_recv: {self.monitor.elapsed('agent:communication_conv_recv')}")
                                                                
                            logger.log("OVERHEAD", f"agent:llm_backend_recv: {self.llm_monitor.elapsed('agent:llm_backend_recv')}")
                            with open("overhead_calander.csv","a",newline="") as f:
                                f.write(f"\nagent:llm_backend_recv: {self.llm_monitor.elapsed('agent:llm_backend_recv')}")
