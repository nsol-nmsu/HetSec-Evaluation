...
async def send(data, port, delay, host_server):
    ...
    with open("MABE_dec.txt", "a") as f:
        f.write(f"{t1 - t0}\n")

    t2 = time.perf_counter()
    _ = real_mabe_encrypt()
    t3 = time.perf_counter()
    with open("MABE_enc-2.txt", "a") as f:
        f.write(f"{t3 - t2}\n")
    ...
    with open("sym_enc_1-2.txt", "a") as f:
        f.write(f"{t5 - t4}\n")
    ...
    with open("s1_s2_time.txt", "a") as f:
...
async def send2(data, port, delay, host_server):
    ...
    _ = real_mabe_encrypt()
    t1 = time.perf_counter()
    with open("MABE_enc-3.txt", "a") as f:
        f.write(f"{t1 - t0}\n")
...
async def main():
    ...
    server1.CreateClientSocket("172.172.235.2", 5007, 1000)
    print("SSS")
