    ...
    while True:
        ...
        try:  
            ...
            # if the client is not asking for an id, nor is it asking for its data, we can assume it has just sent its weights,so we add them to the line            
                ...
                server1_req_time[currentReqID] = time.perf_counter()
...
async def send(data, port, delay, host_server):
    ...
    with open("MABE_enc_para.txt", "a") as f:
        f.write(f"{t1 - t0}\n")

    t2 = time.perf_counter()
    _enc1 = real_sym_encrypt(send_package)
    _enc2 = real_sym_encrypt(send_package)
    _ = real_sym_decrypt(_enc1)
    _ = real_sym_decrypt(_enc2)
    t3 = time.perf_counter()
    with open("sym_enc_para.txt", "a") as f:
        f.write(f"{t3 - t2}\n")
    ...
    print(f"Recvived data from previous request")
    #Message data, requestID
    total_time = end_time - server1_req_time[reqID]
    with open("s1_time_para.txt", "a") as f:
...
async def send2(data, port, delay, host_server):
    ...
    t0 = time.perf_counter()
    _enc_payload = real_sym_encrypt(send_package)
    _ = real_sym_decrypt(_enc_payload)
    t1 = time.perf_counter()
    with open("sym_enc_1-3.txt", "a") as f:
        f.write(f"{t1 - t0}\n")
    ...
    with open("s1_s3_time.txt", "a") as f:
        f.write(f"{total_time}\n")
