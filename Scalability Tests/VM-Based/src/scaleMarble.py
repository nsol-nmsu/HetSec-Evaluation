    ...
    while True:
        if random.randint(0, 1000) > 1000 or len(IDs) == 0:
            ...
            try:
                ...
                print("fail", file=open("ScaleFailMarble.txt",'a'))
                print("fail")
            time.sleep(1)
        else: 
            # Training
            ...
            for epoch in tqdm(range(args.epochs)):
                ...
                w, loss = local_model.update_weights(model=copy.deepcopy(global_models[my_id]), global_round=global_epochs[my_id])
                updated_weights = w
                start_time = time.time()
                client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                client = ssl_context.wrap_socket(client, server_hostname=server)  
                ...
                try:
                    ...
                    end_time1 = time.time()
                    client.close()
                    print(end_time - start_time, file=open("ScaleSendMarble.txt",'a')) 
                    print(end_time1 - start_time1, file=open("ScaleRecvMarble.txt",'a'))   
                    ...
                    print("fail", file=open("ScaleFailM<arble.txt",'a'))
            time.sleep(1)
