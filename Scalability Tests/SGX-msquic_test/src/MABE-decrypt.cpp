...
int main() {
    std::ifstream jsonFile ( "src/public.json", std::ios::in );
    json publicInfo = json::parse( jsonFile );
    pairing_t pairing;
    char param[1024];
    size_t count = fread(param, 1, 1024, fopen("src/a.param","r"));
    ...
    std::ifstream encryptJson ( "src/encrypt.json", std::ios::in );
    json enc_msg = json::parse( encryptJson );

    std::ifstream userInfoJson ( "src/userInfo.json", std::ios::in );
