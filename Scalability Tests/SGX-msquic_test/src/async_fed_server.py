...
def handle_client():                   
    ...
    while True:
        ...
        client_data, addr, awaiting_response = Network.RUDPrecv(s)
        message = 0
        try:
        ...
        if client_data[:5] == b'no id':
            ...
            id1 = id1 + 1
            print("ds", addr)
            Network.break_data(s, addr, global_pickle)
            ...
            for finish in processed:
                if(finish[2] == message):
                    Network.break_data(s, addr, pickle.dumps(finish))
                    #s.sendto(b'complete file sent',addr)
            ...
            if(flag):
                ...
                Network.break_data(s, addr, b'deny')
        ...
        if(len(awaiting_response) >= 1):
            print("responding to turned ")
            s.sendto(b'turn',awaiting_response.pop())
