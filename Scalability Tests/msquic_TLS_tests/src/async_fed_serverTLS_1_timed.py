...
async def handle_client(reader, writer):   
    ...
    # if the client is not asking for an id, nor is it asking for its data, we can assume it has just sent its weights,so we add them to the line            
        ...
        data_map[reqID] = mes
        await send([global_model, my_id1, reqID], 5007, 0.002, writer) 
...
async def send(data, port, delay, host_server):
    ...
    with open("s1_s2_time_TLS.txt", "a") as f:
        f.write(f"{total_time}\n")

    #del(socket1_map[reqID])
    print("Sending 2", my_id2)
    server2_req_time[reqID] = time.perf_counter()
    await send2([global_model, my_id2, reqID], 5008, 0, host_server) 
...
async def send2(data, port, delay, host_server):  
    ...
    reader, writer = await asyncio.open_connection(
       "172.172.235.2", port, ssl=ssl_context_client
    )    
    ...
    with open("s1_s3_time_TLS.txt", "a") as f:
        f.write(f"{total_time}\n")

    #del(socket2_map[reqID])
    #ReqID to streamID
    mes = data_map[reqID]
